...
current_skill_path = "profiles/analyst/skills/implementation-lead/SKILL.md"
...
# Для каждой фазы определяем исполнителя: профиль analyst (блок C, ADR-003),
# навык — по skill_path соответствующей ТФ в mapping
skill_by_tf = {m.get("tf_code"): m for m in mappings}

def resolve_profile(tf_code):
    # Блок C обслуживается профилем analyst (ADR-003)
    if tf_code.startswith("С/"):
        return "analyst"
    if tf_code.startswith("В/"):
        return "analyst"
    return "architect"
...
for i, phase in enumerate(implementation_phases, 1):
    ...
    if "Оценка эффективности" in phase["phase_name"]:
        ...
        # Инструктаж требует human-in-the-loop (User_Operator) + analyst
        alloc = {
            "phase_id": phase["phase_id"],
            "phase_name": phase["phase_name"],
            "profile": "analyst + User_Operator (human-in-the-loop)",
            "skill": "implementation-lead",
            "source_tf": "С/03.7",
            "source_td": phase["source_td"],
        }
    elif "программного обеспечения" in phase["phase_name"]:
        # Выбор/внедрение ПО — analyst по roadmap resource_requirements
        alloc = {
            "phase_id": phase["phase_id"],
            "phase_name": phase["phase_name"],
            "profile": "analyst",
...
body:
  ...
  resource_allocation:
    - phase_id: str
      phase_name: str
      profile: str         # профиль из mapping (analyst / architect / User_Operator)
      skill: str
      source_tf: str
      source_td: str
  risk_management:
