...
KNOWN_PROFILES = ["regulator", "architect", "analyst", ".default"]

def extract_profile_from_skill_path(skill_path, registry_entry=None):
    """Возвращает (profile, skill).

    skill_path в mapping относительный: 'info-gatherer/SKILL.md'.
    Профиль берём из registry (полный путь profiles/<profile>/skills/<skill>/),
    иначе ищем skill_name по всем профилям.
    """
    parts = skill_path.split("/")
    if len(parts) >= 4 and parts[0] == "profiles":
        return parts[1], parts[3]

    skill_name = parts[0].replace("/SKILL.md", "")

    # 1. Из registry — там полный путь с профилем
    if isinstance(registry_entry, dict):
        full = registry_entry.get("skill_path", "")
        fparts = full.split("/")
        if len(fparts) >= 4 and fparts[0] == "profiles" and fparts[3] == skill_name:
            return fparts[1], skill_name

    # 2. Поиск по директориям всех профилей
    profiles_root = os.path.join(ARCH_DIR, "profiles")
    if os.path.isdir(profiles_root):
        for p in sorted(os.listdir(profiles_root)):
            candidate = os.path.join(profiles_root, p, "skills", skill_name, "SKILL.md")
...
current_skill_path = "profiles/analyst/skills/crossfunction-graph-builder/SKILL.md"
