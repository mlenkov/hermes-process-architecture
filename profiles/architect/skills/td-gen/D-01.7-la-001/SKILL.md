...
# Архитектор-скиллы
architect_skills = []
arch_skills_dir = os.path.join(HERMES_DIR, "skills", "architect")
    ...
    for d in sorted(os.listdir(arch_skills_dir)):
        ...
        if os.path.isfile(skill_md):
            ...
            architect_skills.append({
                "skill_id": f"architect/{d}",
                "description": m.group(1).strip() if m else ""
            })

# td-gen скиллы (собственные навыки генерации)
tdgen_skills = []
tdgen_dir = os.path.join(HERMES_DIR, "skills", "architect", "td-gen")
...
# Добавить architect (его нет в AGENTS.md, но он есть в системе)
if not any(p.get("profile_id") == "architect" for p in coordinators):
    coordinators.append({"profile_id": "architect", "name": "Хранитель архитектуры", "role": "Coordinator"})
if not coordinators:
